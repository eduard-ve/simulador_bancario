...
public class CuentaAhorros
{
    ...
    public double calcularSaldoPromedio( int pNumMeses )
    {
        if ( pNumMeses <= 0 )
        {
            return saldo; 
        ...
        // Creamos una variable temporal para el saldo  evitando modficar el saldo real
        double saldoTemporal = saldo;
        double sumaDeSaldos = 0;

        // Simula el paso de cada mes y acumulamos el saldo
        for( int i = 0; i < pNumMeses; i++ )
        {
          
            saldoTemporal = saldoTemporal * ( 1 + interesMensual );  
            sumaDeSaldos += saldoTemporal; 
        }
        
        return sumaDeSaldos / pNumMeses;
    }
}